...
function isPricingIntent(message) {
  ...
  return (
    ...
    /\bto\b/.test(text) ||
    /that route|that trip|same route/.test(text)
  );
}

function isIncidentIntent(message) {
  const text = normalizeText(message);
  return /drunk|refus|pay|accident|fight|robbery|stolen|police|danger|threat|violence|emergency|attack/.test(text);
}
...
async function supabaseFetch(table, limit = 50, query = "") {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    tableDebug[table] = { ok: false, reason: "Missing Supabase env vars" };
    return [];
  }
...
async function supabaseInsert(table, payload) {
  ...
  try {
    const response = await fetch(`${cleanBaseUrl()}/rest/v1/${table}`, {
      method: "POST",
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
...
async function supabaseUpsert(table, payload, conflictColumn) {
  ...
  try {
    const response = await fetch(
      `${cleanBaseUrl()}/rest/v1/${table}?on_conflict=${conflictColumn}`,
      {
        method: "POST",
        headers: {
          apikey: SUPABASE_SERVICE_ROLE_KEY,
    ...
    if (!response.ok) {
      tableDebug[table] = { ok: false, status: response.status, upsert_error: text };
      return null;
    }

    return text ? JSON.parse(text) : null;
  } catch (error) {
    tableDebug[table] = { ok: false, upsert_error: error.message };
    return null;
  }
}
...
async function fetchOperationalMemory() {
  const rows = await supabaseFetch(
    "tapiwa_operational_memory",
    100,
    "&active=eq.true"
  );
...
async function saveTeamChatMessage({ text, risk_level = "low", needs_supervisor = false }) {
  if (!text) return null;

  return supabaseInsert("team_chat", {
    role: "ai",
    author: "Dispatch Tapiwa",
    author_email: "tapiwa@zachangu.local",
    text,
    risk_level,
    needs_supervisor
  });
...
async function saveAuditLog(payload) {
  return supabaseInsert("tapiwa_ai_audit_logs", {
    ...
    source_ref: "server.js persistent-memory-v3"
  });
}
...
function extractRouteParts(message) {
  ...
  if (match) return { pickupRaw: match[1].trim(), dropoffRaw: match[2].trim() };
...
function getLandmarkName(row) {
  return row?.Landmark_Name || row?.landmark_name || row?.name || "";
}
...
function getLatitude(row) {
  return Number(row?.latitude ?? row?.Latitude ?? row?.lat ?? row?.Lat);
}

function getLongitude(row) {
  return Number(row?.longitude ?? row?.Longitude ?? row?.lng ?? row?.Lon ?? row?.long);
}
...
function bestLandmarkMatch(rawName, landmarks) {
  ...
  const candidates = landmarks.map((lm) => {
    ...
    let bestScore = 0;
    for (const name of names) bestScore = Math.max(bestScore, similarity(rawName, name));

    return { landmark: lm, score: bestScore };
  });

  candidates.sort((a, b) => b.score - a.score);
  return { match: candidates[0]?.landmark || null, score: candidates[0]?.score || 0 };
}
...
function buildRouteUnderstanding(message, landmarksRaw, memory = {}) {
  ...
  if (
    ...
    /that route|that trip|same route|it|how much|price|fare|quote|distance|km/.test(text) &&
    ...
    return {
      ...
      note: "Using confirmed route from persistent memory"
    };
  }
...
async function getOrsDistanceKm(pickup, dropoff) {
  if (!ORS_API_KEY || !hasCoords(pickup) || !hasCoords(dropoff)) return null;
...
function findMatchingMarketRule(route, rules) {
  ...
  for (const rule of rules || []) {
    ...
    let score = 0;
    if (rule.pickup_landmark && similarity(pickupName, rule.pickup_landmark) > 0.75) score += 3;
    if (rule.dropoff_landmark && similarity(dropName, rule.dropoff_landmark) > 0.75) score += 3;
...
async function calculateTripQuote({
  ...
  let marketNote = "System pricing engine used.";
  ...
  if (marketRule) {
    ...
    const marketTooLow = marketMax < systemPrice * 0.65 && distanceKm >= 10;
    ...
    if (!marketTooLow) {
      ...
      marketNote = "Market data considered but system economics still controls.";
    } else {
      marketNote =
  ...
  const conditions = [];
  if (peak) conditions.push("peak");
  if (night) conditions.push("night");
  if (rain) conditions.push("rain");
  if (event) conditions.push("event");
  ...
  const result = {
    ...
    range_min: rangeMin,
    range_max: rangeMax,
    confidence: distanceData.source === "ORS" ? "high" : "medium",
    market_note: marketNote,
    operational_memory_used: data.operationalMemory,
    team_message: `Should be around ${money(recommended)} — safe range ${money(rangeMin)} to ${money(rangeMax)}${
...
Keep exact prices. Do not calculate new prices. Do not add facts.
Do not repeat the same safety phrase. Maximum 2 sentences.
            `.trim()
          },
...
app.get("/", (req, res) => {
  res.json({
    status: "Zachangu AI server persistent-memory-v3 is running",
...
app.post("/ai/analyze", async (req, res) => {
  ...
  try {
    ...
    if (isConfirmation(cleanMessage) && memory.pendingRoute) {
      ...
      const team_message = `${getLandmarkName(memory.confirmedRoute.pickup) || memory.confirmedRoute.pickupRaw} to ${
        getLandmarkName(memory.confirmedRoute.dropoff) || memory.confirmedRoute.dropoffRaw
      }, confirmed route.`;

      await saveTeamChatMessage({ text: team_message });
      return res.json({
        ignored: false,
    ...
    if (isPricingIntent(cleanMessage) || isConfirmation(cleanMessage) || isCorrection(cleanMessage)) {
      const quote = await calculateTripQuote({
    ...
    await saveTeamChatMessage({ text: defaultMessage });
    await saveAuditLog({
      user_message: message,
      clean_message: cleanMessage,
    ...
    const fallback = "Something went off on my side — try that again.";
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Zachangu AI server persistent-memory-v3 running on port ${PORT}`);
});
