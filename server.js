...
const allowedCorsOrigins = Array.from(new Set([
  ...defaultCorsOrigins,
  ...parseCsvList(CORS_ALLOWED_ORIGINS)
]));
...
function buildHealthPayload() {
  ...
  return {
    ...
    notifications_provider: "supabase",
...
function normalizeNotificationTarget(raw = {}) {
  ...
  const type = String(
    target.type ||
    body.target_type ||
    (target.email || body.user_email || body.email ? "email" : "") ||
    (target.role || body.role ? "role" : "") ||
    "all"
  ).trim().toLowerCase();
  return {
    type: ["all", "role", "email"].includes(type) ? type : "all",
    role: String(target.role || body.role || "").trim().toLowerCase(),
    email: String(target.email || body.user_email || body.email || "").trim().toLowerCase()
  };
}
  ...
  try {
    ...
    const sanitizedRows = rows
      .map((row) => ({
        ...
        recipient_email: row?.recipient_email || row?.recipientEmail || null,
...
app.post("/notifications/send", requireApiAuth, async (req, res) => {
  try {
    ...
    const type = String(req.body?.notification_type || req.body?.type || "system_alert").trim().toLowerCase() || "system_alert";
    const entityId = req.body?.entity_id || req.body?.entityId || req.body?.tripId || req.body?.trip_id || null;
    const entityType = req.body?.entity_type || req.body?.entityType || req.body?.panel || null;
    const priority = String(req.body?.priority || "normal").trim().toLowerCase() || "normal";
    const baseKey = String(req.body?.idempotency_key || `${type}:${target.type}:${target.role || target.email || "all"}:${title}:${body}:${now.slice(0, 16)}`).slice(0, 240);
    const row = {
      recipient_id: null,
      recipient_email: target.type === "email" ? target.email : null,
      recipient_role: target.type === "role" ? target.role : "all",
      type,
      title,
      body,
      entity_id: entityId,
    ...
    const result = await supabaseWrite("notifications", "POST", [row], { onConflict: "idempotency_key" });
    if (result.error) {
      return res.status(500).json({ ok: false, error: result.error, provider: "supabase", target });
    }
    return res.json({
      ok: true,
      provider: "supabase",
      delivery: "notifications_table",
      target,
      data: Array.isArray(result.data) ? result.data : []
    });
  } catch (error) {
    return res.status(500).json({ ok: false, error: error.message || "Failed to publish Supabase notification." });
  }
});
...
async function supabaseSelect(table, options = {}) {
  if (!SUPABASE_URL || !SUPABASE_API_KEY) return { data: [], error: "Supabase is not configured on the server." };
  const params = ["select=*"];
  ...
  const url = `${cleanBaseUrl()}/rest/v1/${table}?${params.join("&")}`;
  try {
    const response = await fetchWithTimeout(url, {
      method: "GET",
      headers: {
    ...
    const text = await response.text();
    const body = text ? safeJsonParse(text) || text : [];
    if (!response.ok) return { data: [], error: typeof body === "string" ? body : JSON.stringify(body) };
    return { data: Array.isArray(body) ? body : [], error: null };
  } catch (error) {
    return { data: [], error: error.message || "Supabase select failed." };
  }
}
