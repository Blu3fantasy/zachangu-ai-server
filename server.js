...
index c563fc4cdbc1691011c3c052442902d4536a4442..d60f27b3d9348c6b05ff5b16f77286e97c6caa50 100644
--- a/server.js
+++ b/server.js
@@ -1,38 +1,38 @@
...
+
 ...
 function saveConversationMemoryToDisk() {
   try {
@@ -148,64 +148,64 @@ function loadConversationMemoryFromDisk() {
   try {
...
+    confirmedRoute: memory.confirmedRoute || null,
 ...
 async function callMaurice(userMessage, systemContext = {}) {
   ...
   const payload = {
     model: MAURICE_MODEL,
     messages: [
@@ -231,119 +231,124 @@ async function callMaurice(userMessage, systemContext = {}) {
...
+  const text = normalizeText(message);
+  return /\b(yes|yep|yeah|correct|confirmed|confirm|thats right|that's right|exact|exactly|okay|ok|sure)\b/.test(text);
+}
+
+function isFollowUpRouteMessage(message) {
+  const text = normalizeText(message);
...
@@ -445,648 +450,654 @@ function buildLocalTapiwaFallback({ cleanMessage, computedFare, routeUnderstandi
     };
   }
...
+    const hasExplicitRoute = isExplicitRouteMessage(cleanMessage);
+    const isCorrection = isCorrectionMessage(cleanMessage);
+    const isConfirmation = isConfirmationMessage(cleanMessage);
+
+    if (isConfirmation && memory.pendingConfirmation && memory.lastRoute) {
